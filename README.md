...
This creates a Windows ZIP under `artifacts/zamme-aesse/`. The package includes the Stand 11 first-run catalog for Getränkestand «El Tony & mehr» and supports the authenticated CashSloth Server API v1 workflow. Event-package features remain controlled through `CashSloth.Features.json` and the app settings.

Ongoing event testing happens on the public `zaa` branch without creating a release for every change. `main` remains the product source of truth, and `zaa` is never merged back into it; see [Public `zaa` event branch](docs/ZAA_BRANCH.md).
...
- The Zamme Aesse profile keeps its Stand 11 catalog and lean event UI while using the same server v1 client.
