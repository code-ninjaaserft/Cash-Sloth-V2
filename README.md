...
```
This creates a Windows ZIP under `artifacts/zamme-aesse/`. The package includes `CashSloth.Features.json`, which keeps only Shop/catalog, History, and Settings visible, keeps the laptop awake, starts in soft kiosk mode, requires a kiosk exit password, and locks Windows when CashSloth exits. Its first-run cart catalog is preloaded with the Stand 11 offer for Getränkestand «El Tony & mehr». Features can be unlocked on a controlled device by changing that JSON file next to `CSV2.exe` and restarting the app.
