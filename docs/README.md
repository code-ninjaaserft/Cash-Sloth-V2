# Docs

_Last updated: 2026-08-19_

## Current status and planning

- [CSV2 Bucketlist](CSV2_BUCKETLIST.md)
- [CSV2 Server & External Work](CSV2_SERVER_EXTERNAL_BUCKETLIST.md)
- [Public `zaa` event branch](ZAA_BRANCH.md)
- [Roadmap](ROADMAP.md)
- [Milestones](MILESTONES.md)
- [Central server operations](../src/CashSloth.Server/README.md)
- [WPF POS/client behavior](../src/CashSloth.App/README.md)

## Technical references and historical records

- [August persistence kickoff](AUGUST_PERSISTENCE_KICKOFF.md)
- [ABI Contract](ABI.md)
- [QEN-GV MVP acceptance checklist](QEN_GV_MVP_ACCEPTANCE_CHECKLIST.md)
- [QEN-GV release rehearsal checklist](QEN_GV_RELEASE_REHEARSAL_CHECKLIST.md)

Current headline targets:
- **Central Server v1:** implemented; clean-machine and operational validation active.
- **Z'Ämme ässe / August deployment readiness:** target date 2026-08-22.

The QEN-GV and original Mobile Event Rollout documents are historical evidence. Mobile ordering, provider payments, and cross-register synchronization remain open rather than being part of server v1.
